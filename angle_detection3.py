...
for i in range(18):  # 循环次数自己选择
    img = cv2.imread("images/test_%d.bmp" % (i + 1))  # 路径自己选择
    ...
    roi = img_gray[start_y:end_y, start_x:end_x]  # test
    ...
    def draw_line(roi_copy, img_copy, rho, theta):
        a = np.cos(theta)
        b = np.sin(theta)
        x0 = a * rho
        y0 = b * rho
        ...
        cv2.line(roi_copy, (x1, y1), (x2, y2), (255, 255, 0), 10)

        cv2.line(img_copy, (x1 + offset_x, y1 + offset_y),
                 (x2 + offset_x, y2 + offset_y), (0, 255, 0), 10)


    def hough_change(canny_img):
        ...
        lines = cv2.HoughLines(canny_img, 3, np.pi / 18000, 330)
        ...
        if flag_left_up:
            ...
            draw_line(roi_copy, img_copy, rho_left_up_mean, theta_left_up_mean)
        ...
        if flag_left_down:
            ...
            draw_line(roi_copy, img_copy, rho_left_down_mean, theta_left_down_mean)
        ...
        if flag_right_up:
            ...
            draw_line(roi_copy, img_copy, rho_right_up_mean, theta_right_up_mean)
        ...
        if flag_right_down:
            ...
            draw_line(roi_copy, img_copy, rho_right_down_mean, theta_right_down_mean)
        ...
        print("The number %d min angle is: %.2f" % (i + 1, angle_min))
        print("--------------------")
        # print("%.2f" % angle_min)
        # f = open('/test.txt', 'w')
