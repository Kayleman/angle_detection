...
# canny_left = roi_canny[0: height, 0: width / 2]
# canny_right = roi_canny[0: height, width / 2: width]
...
def hough_change(canny_img):
    ...
    lines = cv2.HoughLines(canny_img, 3, np.pi / 18000, 300)
    ...
    for rho, theta in lines2d[:]:
        ...
        if 120 < angle_line < 140:
            angle_left.append(angle_line)
        elif 30 < angle_line < 50:
            if rho < 640:
                angle_right_1.append(angle_line)
            else:
                angle_right_2.append(angle_line)
    ...
    angle_res_1 = 180 - (angle_left_mean - angle_right_mean_1)
    angle_res_2 = 180 - (angle_left_mean - angle_right_mean_2)
    ...
    return roi_copy, img_copy, angle_res_1, angle_res_2


# line_left, theta1 = hough_change(canny_left)
# line_right, theta2 = hough_change(canny_right)
roi_line, img_line, angle_1, angle_2 = hough_change(roi_canny)
print("This angle is :", angle_1, angle_2)
...
plt.subplot(223), plt.imshow(roi, 'gray'), plt.title('ROI of Image')
plt.xticks(())  # ignore xticks
plt.yticks(())  # ignore yticks
